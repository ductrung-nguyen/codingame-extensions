...
if (chrome?.tabs) {
  ...
  chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
    if (ideTabs.has(tabId) && changeInfo?.status === 'loading' && changeInfo?.url && !isCodinGameUrl(changeInfo.url)) {
      unregisterIdeTab(tabId);
    }
  });
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message?.type === 'battle_response_captured') {
    ...
    console.log('[BATTLE] Received battle_response_captured from content script');
...
function connectToBridge(force = false) {
  ...
  cleanupSocket();
  const retryDelay = RECONNECT_SCHEDULE_MS[Math.min(reconnectAttempts, RECONNECT_SCHEDULE_MS.length - 1)];
  if (!force && reconnectAttempts > 0) {
    state.retrying = true;
    state.retryDelay = retryDelay;
    console.warn(`[bridge] reconnect attempt #${reconnectAttempts}, delay ${retryDelay}ms`);
...
function openSocket() {
  ...
  ws.onerror = (event) => {
    const errorMsg = event?.error?.message || event?.message || 'WebSocket connection failed';
    recordError(errorMsg);
  };

  ws.onclose = (event) => {
    console.warn('[bridge] websocket closed', { code: event.code, reason: event.reason });
    ...
    // 3. Going away (1001)
    const shouldReconnect = !intentionalDisconnect && event.code !== 1000 && event.code !== 1001;

    if (shouldReconnect) {
      scheduleReconnect();
      pushLog('warn', 'Bridge disconnected, reconnecting...');
    } else {
      console.info('[bridge] Normal closure or intentional disconnect, not reconnecting');
      pushLog('info', event.code === 1000 ? 'Replaced by another client' : 'Bridge closed');
...
function handleBridgeMessage(raw) {
  ...
  if (message.type === 'auth_ack') {
    if (message.payload?.status === 'success') {
      ...
      pushLog('error', `Authentication failed: ${message.payload?.reason || 'unknown'}`);
      state.lastError = {
...
function sendHello() {
  ...
  if (!state.token) {
    ...
    console.error('[bridge] please configure the auth token in extension options');
    return;
  }
...
function startHeartbeat() {
  ...
  heartbeatTimer = setInterval(() => {
    ...
    sendBridgeMessage({ type: 'heartbeat', timestamp: new Date().toISOString() });
...
function sendBridgeMessage(payload) {
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    console.warn('[bridge] cannot send, socket not open. readyState:', ws?.readyState);
    return false;
  }
  try {
    ...
    console.log('[bridge] Sending message type:', message.type, 'deliveryId:', message.deliveryId);
...
function normalizeMatchPayload(body, contextUserId, contextGameId) {
  ...
  console.log('[BATTLE] Context - userId:', contextUserId, 'gameId:', contextGameId);
    ...
    if (maxDepth > 0 && keys.length > 0 && keys.length < 20) {
      keys.forEach(key => {
        ...
        if (val && typeof val === 'object' && !Array.isArray(val)) {
          ...
          console.log(`[BATTLE]   ${name}.${key}:`, typeof val === 'string' && val.length > 100 ? `${val.substring(0, 100)}...` : val);
        }
      });
    }
  };
  ...
  if (data?.gameId && data?.scores && data?.ranks) {
    ...
    if (playerIndex === null && contextUserId && data.agents) {
      for (let i = 0; i < data.agents.length; i++) {
        if (data.agents[i] && data.agents[i].codingamer && data.agents[i].codingamer.userId === contextUserId) {
          playerIndex = i;
          console.log('[BATTLE] Player index from agents array matching userId:', playerIndex);
          break;
        }
      }
    }
    ...
    if (playerIndex === null && data.metadata && typeof data.metadata === 'object') {
      if (typeof data.metadata.playerIndex === 'number') {
        playerIndex = data.metadata.playerIndex;
        console.log('[BATTLE] Player index from metadata.playerIndex:', playerIndex);
      } else if (typeof data.metadata.agentId === 'number') {
        playerIndex = data.metadata.agentId;
        console.log('[BATTLE] Player index from metadata.agentId:', playerIndex);
      }
    }
    ...
    if (typeof playerIndex !== 'number' || playerIndex < 0 || playerIndex > 1) {
      console.warn('[BATTLE] Invalid player index:', playerIndex, '- defaulting to 0');
      playerIndex = 0;
    }
    ...
    console.log(`[BATTLE] Player index: ${playerIndex}, My rank: ${myRank}, Opponent rank: ${opponentRank}, Result: ${result}`);
    console.log(`[BATTLE] Player scores: [${data.scores[playerIndex]}] vs [${data.scores[opponentIndex]}]`);
  ...
  if (!match) {
    console.warn('[BATTLE] No match object found, inspecting response structure:');
  ...
  const orderCandidate =
    Number.isInteger(data?.startingIndex) ? data.startingIndex : match.playerIndex;
  const order = orderCandidate === 0 ? 0 : 1;
  const stdout =
    match.stdout || data?.stdout || data?.logs?.stdout || '';
  const stderr =
    match.stderr || data?.stderr || data?.logs?.stderr || '';
  const timestamp = new Date().toISOString();
  return {
...
function initializeRetryQueue() {
  ...
  chrome.storage.local.get([STORAGE_QUEUE_KEY], (result) => {
    ...
    const savedEntries = Array.isArray(result?.[STORAGE_QUEUE_KEY]) ? result[STORAGE_QUEUE_KEY] : [];
...
function manualRetryAll() {
  ...
  scheduleRetryProcessing(0);
  return count;
  console.info('[retry] manual resend triggered', { count: retryQueue.size, manual_retry: true });
}
...
function processRetryQueue() {
  ...
  for (const entry of retryQueue.values()) {
    if (entry.nextAttemptTs > now) {
      nextWake = nextWake === null ? entry.nextAttemptTs : Math.min(nextWake, entry.nextAttemptTs);
      continue;
    }
    const message = {
      type: 'match_data',
    ...
    if (sent) {
      console.info(`[retry] sent payload ${entry.deliveryId}`, { manual_retry: Boolean(entry.manualRetry) });
    ...
    const delay = RETRY_SCHEDULE_MS[Math.min(entry.attempts, RETRY_SCHEDULE_MS.length - 1)];
...
function dispatchSyncCode(message) {
  ...
  const timeoutId = setTimeout(() => handleSyncTimeout(requestId), SYNC_RESPONSE_TIMEOUT_MS);
  ...
  chrome.tabs.sendMessage(targetTabId, { type: 'sync_code', requestId, payload }, (response) => {
    ...
    if (lastErr) {
      handleSyncFailure(requestId, 'EDITOR_UNRESPONSIVE', lastErr.message || 'Content script unreachable', meta);
      return;
    }
    if (!response || response.type !== 'sync_status') {
      handleSyncFailure(requestId, 'INVALID_SYNC_RESPONSE', 'Content script did not return sync_status', meta);
      return;
    }
...
function handleSyncTimeout(requestId) {
  ...
  handleSyncFailure(requestId, 'EDITOR_TIMEOUT', 'Content script response timed out', meta);
}
...
function emitSyncStatus(message) {
  ...
  pushLog('warn', `[sync] cached status:${message.requestId}`, { reason: 'BRIDGE_OFFLINE' });
}
...
function registerIdeTab(tab, payload = {}) {
  if (!tab?.id || !isCodinGameUrl(tab.url)) {
    console.log('[TAB] Skipping tab registration - invalid tab or non-CodinGame URL', {
      tabId: tab?.id,
      url: tab?.url
    });
    return;
...
self.debugCG = {
  ...
  checkContentScript: async () => {
    ...
    const tabs = await chrome.tabs.query({ url: 'https://www.codingame.com/*' });
    console.log('Found', tabs.length, 'CodinGame tabs');

    for (const tab of tabs) {
      console.log(`\nChecking tab ${tab.id}:`, tab.url);
      try {
        const response = await chrome.tabs.sendMessage(tab.id, { type: 'ping_editor' });
...
console.log('  self.debugCG.showState()          - Show current extension state');
console.log('  self.debugCG.listTabs()           - List all registered IDE tabs');
console.log('  self.debugCG.testSync()           - Send test sync command');
console.log('  self.debugCG.forceRegister()      - Manually register current tab');
console.log('  self.debugCG.clearState()         - Clear all state');
console.log('  self.debugCG.checkContentScript() - Check content script status');
console.log('');
...
chrome.storage.local.get(['cg_auth_token'], (result) => {
  if (result.cg_auth_token) {
    ...
    console.warn('[bridge] no auth token found in storage - connection will fail');
    console.warn('[bridge] please configure the auth token in extension options');
    pushLog('warn', 'No auth token configured - please set token in extension options');
  }
});
