...
(function () {
  ...
  console.log('[CodinGame Page Context] Angular available:', typeof window.angular);
  console.log('[CodinGame Page Context] Monaco available:', typeof window.monaco);
  ...
  window.__cgTestSync = function (code) {
    ...
    const event = new CustomEvent('__cgTestSync', {
      detail: {
        code: code || '// TEST CODE FROM CONSOLE\npackage main\n\nfunc main() {\n    println("Test from page context!")\n}\n',
        language: 'Go'
      }
    });
  ...
  window.__cgSyncCode = function (code) {
    console.log('[CodinGame Page Context] __cgSyncCode() called with', code.length, 'characters');
    ...
    try {
      ...
      console.log('[CodinGame Page Context] Current value:', ngModel.$viewValue?.substring(0, 50) + '...');
      ...
      try {
        ...
        if (scope && scope.$apply) {
          scope.$apply();
          console.log('[CodinGame Page Context] ✅ Angular $apply() called via element scope');
        ...
        try {
          ...
          if (injector) {
            ...
            if ($rootScope && $rootScope.$apply) {
              $rootScope.$apply();
              console.log('[CodinGame Page Context] ✅ Angular $apply() called via $rootScope');
              digestTriggered = true;
            }
          }
        } catch (e) {
      ...
      if (!digestTriggered) {
        console.warn('[CodinGame Page Context] ⚠️ Could not trigger $apply, forcing $render only');
      }

      console.log('[CodinGame Page Context] ✅ Code updated successfully via Angular!');
      console.log('[CodinGame Page Context] New value:', ngModel.$viewValue?.substring(0, 50) + '...');
  ...
  window.__cgGetCurrentLanguage = function () {
    ...
    try {
      ...
      if (codeEditorElement && typeof window.angular === 'object') {
        ...
        if (scope && scope.question && scope.question.programmingLanguageId) {
          const langId = normalizeLanguage(scope.question.programmingLanguageId);
          console.log('[CodinGame Page Context] Language from Angular scope:', langId);
          return langId;
        }
      }
    } catch (e) {
      console.log('[CodinGame Page Context] Could not get language from Angular:', e.message);
    }
    ...
    try {
      ...
      console.log('[CodinGame Page Context] Could not get language from URL:', e.message);
    }

    // Method 3: Try to get from language selector dropdown
    try {
      const langSelector = document.querySelector('.language-select, [data-language], .cg-ide-language-selector');
      if (langSelector) {
      ...
      console.log('[CodinGame Page Context] Could not get language from selector:', e.message);
    }
  ...
  window.addEventListener('__cgSyncCodeRequest', (event) => {
    console.log('[CodinGame Page Context] Received sync request from content script');
    ...
    console.log('[CodinGame Page Context] Sent sync response to content script');
  });
  ...
  window.addEventListener('__cgDetectLanguage', () => {
    ...
    window.dispatchEvent(responseEvent);
    console.log('[CodinGame Page Context] Sent language detection response:', language);
  });
  ...
  function notifyBattleCapture(url, body) {
    ...
    console.log('[CodinGame Page Context] Body preview:', body?.substring(0, 200));
    ...
    window.dispatchEvent(event);
    console.log('[CodinGame Page Context] Battle response event dispatched to content script');
  }
  ...
  window.fetch = function (...args) {
    ...
    if (method === 'POST' && url && url.includes('/services/TestSession/startTestSession')) {
        ...
        if (response.ok) {
          ...
          try {
            ...
            if (data && data.handle) {
              ...
              console.log('[CodinGame Page Context] Cached testSessionHandle from startTestSession:', cachedTestSessionHandle);
            }
          } catch (err) {
            console.warn('[CodinGame Page Context] Failed to parse startTestSession response:', err);
          }
        }
        return response;
      });
    ...
    if (method === 'POST' && isBattleEndpoint(url)) {
      ...
      return originalFetch.apply(this, args).then(async (response) => {
        console.log('[CodinGame Page Context] Battle response received, ok:', response.ok);
        if (response.ok) {
          const clonedResponse = response.clone();
          try {
            const body = await clonedResponse.text();
            console.log('[CodinGame Page Context] Battle response body read, notifying...');
            notifyBattleCapture(url, body);
          } catch (err) {
            console.error('[CodinGame Page Context] Failed to read response:', err);
          }
        }
        return response;
      });
  ...
  XMLHttpRequest.prototype.send = function (...args) {
    if (this._method?.toUpperCase() === 'POST' && isBattleEndpoint(this._url)) {
      console.log('[CodinGame Page Context] Intercepting battle XHR POST to:', this._url);
      this.addEventListener('load', function () {
        console.log('[CodinGame Page Context] XHR battle response received, status:', this.status);
        if (this.status === 200) {
          try {
            const body = this.responseText;
            console.log('[CodinGame Page Context] XHR response body read, notifying...');
            notifyBattleCapture(this._url, body);
          } catch (err) {
            console.error('[CodinGame Page Context] Failed to read XHR response:', err);
          }
        }
      });
    }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.data.type === '__cgGetUserId') {
      ...
      if (!userId) {
        try {
          if (window.session && window.session.codinGamer && window.session.codinGamer.userId) {
            userId = window.session.codinGamer.userId;
            console.log('[CodinGame Page Context] Found user ID from window.session.codinGamer:', userId);
          }
        } catch (e) {
          console.warn('[CodinGame Page Context] Could not get userId from window.session:', e);
        }
      }
      ...
      if (!userId) {
        try {
          const analyticsData = localStorage.getItem('cgAnalyticsUserProperties.codingame');
          if (analyticsData) {
            const parsed = JSON.parse(analyticsData);
            if (parsed && parsed['Handle ID']) {
              userId = parsed['Handle ID'];
              console.log('[CodinGame Page Context] Found user ID from localStorage analytics:', userId);
            }
          }
        } catch (e) {
          console.warn('[CodinGame Page Context] Could not get userId from localStorage:', e);
        }
      }
        ...
        try {
          if (typeof window.angular !== 'undefined') {
            ...
            if (scope && scope.user && scope.user.id) {
              ...
              console.log('[CodinGame Page Context] Found user ID from Angular scope:', userId);
            }
          }
        } catch (e) {
          console.warn('[CodinGame Page Context] Could not get userId from Angular:', e);
        }
      }
      ...
      if (!userId) {
        try {
          const globalUserVars = ['CodinGamer', 'cgUser', 'currentUser', 'codinGamer'];
          for (let varName of globalUserVars) {
            if (window[varName] && window[varName].userId) {
              userId = window[varName].userId;
              console.log('[CodinGame Page Context] Found user ID from window.' + varName + '.userId:', userId);
              break;
            }
            if (window[varName] && window[varName].id) {
              userId = window[varName].id;
              console.log('[CodinGame Page Context] Found user ID from window.' + varName + '.id:', userId);
              break;
            }
          }
        } catch (e) {
          console.warn('[CodinGame Page Context] Could not get userId from global variables:', e);
        }
      }

      // If still no userId, try API call as last resort
      if (!userId) {
        console.log('[CodinGame Page Context] Attempting API call to get userId...');
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (data && data.userId) {
                userId = data.userId;
                cachedUserId = userId;
                console.log('[CodinGame Page Context] ✅ Found userId from getCurrentCodinGamer:', userId);
                window.postMessage({ type: '__cgUserIdResponse', userId }, '*');
                return;
              }
            }
          } catch (err) {
            console.warn('[CodinGame Page Context] getCurrentCodinGamer failed:', err.message);
          }

          // Fallback: try to get from session
          try {
            const response = await fetch('https://www.codingame.com/services/session/findSession', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
            ...
            if (response.ok) {
              ...
              if (data && data.userId) {
                userId = data.userId;
                cachedUserId = userId;
                console.log('[CodinGame Page Context] ✅ Found userId from session:', userId);
                window.postMessage({ type: '__cgUserIdResponse', userId }, '*');
                return;
              }
            }
          } catch (err) {
            console.warn('[CodinGame Page Context] findSession failed:', err.message);
          }

          // All methods failed
          console.warn('[CodinGame Page Context] Could not find userId from any source');
    ...
    if (event.data.type === '__cgGetTestSessionHandle') {
      console.log('[CodinGame Page Context] Received request for test session handle');
        ...
        try {
          if (typeof window.angular !== 'undefined') {
            ...
            if (scope && scope.testSession && scope.testSession.handle) {
              ...
              console.log('[CodinGame Page Context] Found test session handle from Angular scope:', handle);
            }
          }
        } catch (e) {
          console.warn('[CodinGame Page Context] Could not get test session handle from Angular:', e);
        }
      }
      ...
      if (!handle && window.testSession && window.testSession.handle) {
        ...
        console.log('[CodinGame Page Context] Found test session handle from global:', handle);
      }
      ...
      if (handle) {
        console.log('[CodinGame Page Context] Responding with testSessionHandle:', handle);
        cachedTestSessionHandle = handle;
      } else {
        console.warn('[CodinGame Page Context] Could not find testSessionHandle from any source');
      }

      window.postMessage({ type: '__cgTestSessionHandleResponse', handle }, '*');
    }
    ...
    if (event.data.type === '__cgGetBattles') {
      ...
      try {
        ...
        if (typeof window.angular !== 'undefined') {
          console.log('[CodinGame Page Context] Angular is available, searching for battles...');
          ...
          for (const selector of selectors) {
            ...
            console.log(`[CodinGame Page Context] Selector "${selector}" found ${elements.length} elements`);
            ...
            for (const element of elements) {
              ...
              if (scope) {
                console.log(`[CodinGame Page Context] Scope found for ${selector}, inspecting...`);
                ...
                while (currentScope && depth < maxDepth) {
                  ...
                  const battlesData = currentScope.battles ||
                  ...
                  if (Array.isArray(battlesData) && battlesData.length > 0) {
                    console.log(`[CodinGame Page Context] Found battles array at depth ${depth}:`, battlesData.length);

                    battlesData.forEach((battle, index) => {
                      const gameId = battle.gameId || battle.id || battle.battleId || battle.gameResultId;
                      if (gameId) {
                        battles.push({ gameId: String(gameId) });
                        console.log(`[CodinGame Page Context] Found battle ${battles.length}: ${gameId}`);
                        foundBattles = true;
                      } else {
                        // Log structure for debugging
                        if (index === 0) {
                          console.log('[CodinGame Page Context] Battle object keys:', Object.keys(battle));
                        }
                      }
                    });
          ...
          if (battles.length === 0) {
            console.log('[CodinGame Page Context] Trying body scope as last resort...');
            const bodyElement = window.angular.element(document.body);
            const bodyScope = bodyElement.scope();

            if (bodyScope) {
              // First, log all top-level properties for debugging
              console.log('[CodinGame Page Context] Body scope top-level properties:', Object.keys(bodyScope).filter(k => !k.startsWith('$')));

              // Search all properties recursively (limited depth)
              const searchScope = (obj, depth = 0, maxDepth = 3, path = 'root') => {
                ...
                for (const key in obj) {
                  ...
                  if (Array.isArray(value) && value.length > 0) {
                    ...
                    const first = value[0];
                    console.log(`[CodinGame Page Context] Found array at ${currentPath} with ${value.length} items, first item keys:`, first ? Object.keys(first).slice(0, 10) : 'null');

                    if (first && (first.gameId || first.id || first.battleId || first.gameResultId)) {
                      console.log(`[CodinGame Page Context] ✅ Found battles array in property: ${currentPath}`);
                      value.forEach(battle => {
                        const gameId = battle.gameId || battle.id || battle.battleId || battle.gameResultId;
                        if (gameId) {
                          battles.push({ gameId: String(gameId) });
                          console.log(`[CodinGame Page Context] Added battle: ${gameId}`);
                        }
                      });
                      return true;
                    }
                  } else if (typeof value === 'object' && value !== null) {
                    if (searchScope(value, depth + 1, maxDepth, currentPath)) return true;
                  }
                }
                return false;
              };
        ...
        console.log(`[CodinGame Page Context] Extracted ${battles.length} battles from Angular scope`);
      } catch (e) {
  ...
  console.log('[CodinGame Page Context] ✅ Session info extraction helpers ready');
})();
