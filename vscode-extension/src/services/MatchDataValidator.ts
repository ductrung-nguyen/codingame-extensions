...
export class MatchDataValidator {
  ...
  private validateConstraints(data: Record<string, unknown>): string[] {
    ...
    if ("result" in data && typeof data.result === "string") {
      ...
      if (!validResults.includes(data.result)) {
        errors.push(
          `Invalid result: ${data.result} (must be WIN, LOSE, or DRAW)`,
        );
      }
    }
    ...
    if ("timestamp" in data && typeof data.timestamp === "string") {
      if (!this.isValidTimestamp(data.timestamp)) {
        errors.push(
          `Invalid timestamp format: ${data.timestamp} (must be ISO 8601)`,
        );
      }
    }
  ...
  private normalizePayload(data: Record<string, unknown>): MatchPayload {
    ...
    if (
      "gameResult" in data &&
      typeof data.gameResult === "object" &&
      data.gameResult !== null
    ) {
      normalized.gameResult = this.normalizeGameResult(
        data.gameResult as Record<string, unknown>,
      );
    }

    if ("logs" in data && typeof data.logs === "object" && data.logs !== null) {
      normalized.logs = this.normalizeLogs(
   ...
   */
  private normalizeGameResult(
    gameResult: Record<string, unknown>,
  ): MatchPayload["gameResult"] {
    const normalized: MatchPayload["gameResult"] = {};

    if (
      "scores" in gameResult &&
      typeof gameResult.scores === "object" &&
      gameResult.scores !== null
    ) {
  ...
  private isValidTimestamp(timestamp: string): boolean {
    try {
      ...
      return (
        !isNaN(date.getTime()) &&
        date.toISOString().startsWith(timestamp.substring(0, 10))
      );
    } catch {
      return false;
    }
